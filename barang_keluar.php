...
<head>
    ...
    <title>Material Keluar - Sistem Gudang</title>
    ...
    <style>
        ...
        /* Supaya dropdown akun kanan atas tidak ketutup konten */
        ...
        .table-keluar td:nth-child(4):hover {
            white-space: normal;
...
<body>
    ...
    <nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
        ...
        <div class="ml-auto d-flex align-items-center">
            <div class="dropdown">
                <button class="btn btn-link text-white dropdown-toggle" type="button" id="userDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    ...
    <div class="sidebar">
        <div class="brand-sidebar"><i class="fas fa-warehouse text-primary"></i><span>SISTEM GUDANG</span></div>
        <nav>
            <?php if ($_SESSION['level'] == 'admin'): ?>
            ...
            <?php if ($_SESSION['level'] == 'admin'): ?>
                ...
                <a href="barang_keluar.php" class="active"><i class="fas fa-arrow-up"></i> Material Keluar</a>
            <?php endif; ?>
            <hr>
    ...
    <div class="content">
        <div class="page-header mb-4">
            <h1 class="page-title">Material Keluar ke Cabang</h1>
            <p class="page-subtitle">Halaman untuk mencatat pengiriman material dari gudang ke cabang.</p>
        </div>
        <div class="card mb-4">
            <div class="card-header bg-warning">
                <i class="fas fa-truck"></i> Form Material Keluar
            </div>
            <div class="card-body">
                <form method="post" action="proses_keluar_gudang.php">
                    <div class="row">
                        <div class="col-md-3">
                            <label>Tanggal</label>
                            <input type="text" name="tanggal" class="form-control tanggal" placeholder="Hari/Bulan/Tahun" required>
                        </div>
                        <div class="col-md-3">
                            ...
                            <select name="id_material" id="id_material" class="form-control" required>
                                ...
                                ");
                                    while ($m = mysqli_fetch_assoc($material)) {
                                        echo "<option value='{$m['id_material']}' data-stok='{$m['stok']}'>{$m['kode_material']} - {$m['nama']} (Stok: {$m['stok']} {$m['satuan']})</option>";
                                    }
                                    ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-2">
                            ...
                            <div class="input-group">
                                <select name="id_tujuan" class="form-control" required>
                                    ...
                                    <?php
                                    $cabang = mysqli_query($conn, "SELECT * FROM tujuan WHERE nama IN ('Out - Har', 'Out - PRC', 'Out - DGA 1', 'Out - DGA 2') ORDER BY nama ASC");
                                    while ($c = mysqli_fetch_assoc($cabang)) {
                        ...
                        <div class="col-md-2">
                            <label>&nbsp;</label>
                            <button type="submit" name="barang_keluar" class="btn btn-warning btn-block text-white">
                                Kirim
                            </button>
                        </div>
                    </div>
        ...
        <!-- Riwayat Material Keluar -->
        <div class="card">
            <div class="card-header bg-info text-white">
                <i class="fas fa-history"></i> Riwayat Material Keluar dari Gudang
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-keluar" id="dataTable" width="100%">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>Tanggal</th>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?= $data['qty']; ?> <?= htmlspecialchars($data['satuan']); ?></td>
                                    <td><?= htmlspecialchars($data['tujuan']); ?></td>
                                    <td>
                                        <button class="btn btn-sm btn-danger" onclick="hapusTransaksi(<?= $data['id_transaksi']; ?>, <?= $data['id_material']; ?>, <?= $data['qty']; ?>, <?= $data['id_tujuan']; ?>)">
    ...
    <script>
        ...
        function hapusTransaksi(id, id_material, qty, id_tujuan) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = 'hapus_transaksi.php?type=keluar_gudang&id=' + id + '&id_material=' + id_material + '&qty=' + qty + '&id_tujuan=' + id_tujuan;
                }
            });
        }

        // Pastikan dropdown akun kanan atas aktif
